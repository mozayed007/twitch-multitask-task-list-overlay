...
@app.get("/api/token/refresh/{refresh_token}")
async def proxy_twitch_token_refresh(refresh_token: str):
    """Proxy a refresh-token exchange to twitchtokengenerator.com.

    This avoids CORS problems when the OBS browser source loads the overlay
    from a local file and needs to refresh a token generated by
    twitchtokengenerator.com.
    """
    async with httpx.AsyncClient() as client:
        response = await client.get(
            f"https://twitchtokengenerator.com/api/refresh/{refresh_token}",
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=f"Invalid JSON from token refresh service: {exc}",
        ) from exc
